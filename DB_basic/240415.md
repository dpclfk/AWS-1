# Relational

```sql
CREATE TABLE user(
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  user_id VARCHAR(20) NOT NULL UNIQUE,
  password VARCHAR(64) NOT NULL,
  name VARCHAR(10) NOT NULL,
  phone VARCHAR(13) UNIQUE
);
```

```sql
INSERT INTO user (user_id, password, name) VALUES('text','1234','이정배'), ('carrot','4321','박성민');
```

## 게시판 생성

```sql
CREATE TABLE board(
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  title VARCHAR(20) NOT NULL,
  created_at DATETIME DEFAULT NOW(),
  writer INT UNSIGNED NOT NULL,
  content VARCHAR(10000) NOT NULL
);
INSERT INTO board(title, writer, content) VALUES('타이틀입니다', 1, '첫글');
SELECT * FROM baord; -- 작성자의 id를 찾을 수 있다.
SELECT * FROM user WHERE id=1; -- 관계를 맺으려면 반드시 유니크가 있어야됨(하나만 찾아올 수 있기때문)
```

## user 삭제

```sql
DROP TABLE user;
```

## board와 user의 관계

```sql
DROP TABLE IF EXISTS board;

CREATE TABLE board(
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  title VARCHAR(20) NOT NULL,
  created_at DATETIME DEFAULT NOW(),
  writer INT UNSIGNED,
  content VARCHAR(10000) NOT NULL,
  FOREIGN KEY (writer) REFERENCES user(id)
  -- 외래키FOREIGN KEY)를 사용하여 관계를 DB에게 알려줌
  -- writer에 외래키를 가져온다 user의 id에서
  ON UPDATE CASCADE -- user가 수정되면 writer 똑같이 수정
  ON DELETE SET NULL -- user가 지우면 writer에다가 null
);
ALTER TABLE board ADD CONSTRAINT 'board_user_id' FOREIGN KEY (writer) REFERENCES user(id);
```

# ERD

- Entity Relationship Diagram

## 1 : N || 1 : 다

```mermaid
erDiagram
user {
  UNSIGNED_INT id PK "auto_increment"
  VARCHAR(20) user_id UK "not null"
  VARCHAR(64) password "not null"
  VARCHAR(10) name "not null"
  VARCHAR(13) phone UK
}
board{
  UNSIGNED_INT id PK "auto_increment"
  VARCHAR(20) title "not null"
  CARCHAR(10000) content "not null"
  UNSIGNED_INT writer FK
  DATETIME create_at "default now()"
}
user ||--o{ board: writer
address {
UNSIGNED_INT id PK "auto_increment"
VARCHAR(20) address
UNSIGNED_INT user_id FK
}
user ||--o{ address : user_id
```

## 1 : 1

```mermaid
erDiagram
user {
  UNSIGNED_INT id PK "auto_increment"
  VARCHAR(20) user_id UK "not null"
  VARCHAR(64) password "not null"
  VARCHAR(10) name "not null"
  VARCHAR(13) phone UK
}
user_email {
  UNSIGNED_INT id PK, FK
  VARCHAR(64) email "not null"
}
user ||--o| user_email : id

user_nick {
  UNSIGNED_INT id PK, FK
  VARCHAR(64) nick "not null"
  UNSIGNED_INT user_id FK, UK
}
user ||--o| user_nick : user_id

```

## 다 : 다 || N : M

```mermaid
erDiagram
user {
  UNSIGNED_INT id PK "auto_increment"
  VARCHAR(20) user_id UK "not null"
  VARCHAR(64) password "not null"
  VARCHAR(10) name "not null"
  VARCHAR(13) phone UK
}
follow {
  UNSIGNED_INT follower FK, PK
  UNSIGNED_INT following FK, PK
}
user ||--o{ follow : follower
user ||--o{ follow : following

hobby {
  UNSIGNED_INT id PK "auto_increment"
  VARCHAR(20) name UK "not null"
  VARCHAR(10000) information "not null"
}

user_hobby {
  UNSIGNED_INT user FK
  UNSIGNED_INT hobby FK
}
user ||--o{ user_hobby : user
hobby ||--o{ user_hobby : hobby
```

```sql
CREATE TABLE hobby(
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  name VARCHAR(20) UNIQUE NOT NULL,
  information VARCHAR(10000) NOT NULL
);
  FOREIGN KEY (name) REFERENCES user(id)

```

`SELECT count(*) FROM category_test AS CategoryTest 
LEFT OUTER JOIN category_test AS children 
ON CategoryTest.id = children.category_id 
WHERE CategoryTest.id = 1`
