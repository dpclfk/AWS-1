...
// 상수인 변수를 선언하여 식별자를 parseHeader로 만들고 함수를 할당했다.
...
// parseBody식별자를 가진 함수를 선언 및 할당했다.
// 상수인 변수를 선언하여 식별자를 parseBody로 만들고 함수를 할당했다.
const parseBody = (str) => {
...
// 상수인 변수를 선언하여 식별자를 makeReq로 만들고 함수를 할당했다.
const makeReq = (data) => {
...
//객체를 만들고 그 메서드(함수인 프로터리)로 makeReq를 넣어주고 반환한다.
// require 함수로 호출 됐을때
