...
// import key from "../pem/privatekey.pem";
const publickey = fs.readFileSync(path.join(__dirname, "../pem/publickey.pem"));
const privatekey = fs.readFileSync(path.join(__dirname, "../pem/privatekey.pem"));
...
// publickey로 암호화 => privatekey로 복호회
let encryptPublic = encryptPublicRSA("안녕하세요");
console.log("encryptPublic : ", encryptPublic);
let decryptPrivate = decryptPrivateRSA(encryptPublic);
console.log("decryptedPrivate : ", decryptPrivate);

// privatekey로 암호화 => publickey로 복호화
let encryptedprivate = encryptPrivateRSA("안녕하세요2");
console.log("encryptPrivate : ", encryptedprivate);
